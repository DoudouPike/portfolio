...
class ProjectManager
{
	...
	*/
	...
	public function save(Project $project)
	{
		...
		{
			$query = "UPDATE projects SET title='".$title."', content='".$content."', image='".$image."', url='".$url."', date='".$date."' WHERE id=".$id."";
			$res = mysqli_query($this->db, $query);
			if (!$res)
	...
	public function create($title, $content, $image, $url, $date)
	{
		...
		if(isset($_SESSION["id"]) && $_SESSION["admin"] === "1")
		{
			$query = "INSERT INTO projects (title, content, image, url, date) VALUES('".$title."', '".$content."', '".$image."', '".$url."', '".$date."')";
			$res = mysqli_query($this->db, $query);
			if (!$res)
				throw new Exception("Erreur interne > ".mysqli_error($this->db));
